...
function removeBG(buffer) {
    return new Promise(async (resolve, reject) => {
        ...
        const { data } = await axios.post(`https://xzn.wtf/api/removebg`, { "url": `data:image/jpeg;base64,${file.data.toString("base64")}`, " apikey": "zexxabot" })
        resolve(Buffer.from(data.result.split`,`[1], "base64"))
    })
}
async function DelBg(buffer) {
